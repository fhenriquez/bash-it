...
# Set color for python virtual environment
function set_venv_color() {
  if [ ! -z "$VIRTUAL_ENV" ]
  then
    local env=$VIRTUAL_ENV
    echo "(${lime_yellow}${env##*/}${normal})"
  fi
}
# Set different username color for openstack users vs admin
function set_openstack_user_color() {
  case $(echo $OS_USERNAME) in
    virtadmin) 
      printf "${red}"
      ;;
    *)
...
# Set different username color for users and root
function set_openstack_env_color() {
  case $(echo $OS_REGION) in
    lab) 
      printf "${red}"
      ;;
    *)
...
function prompt_command() {
  ...
  ps_uh_separator="${powder_blue}@${normal}"
  ...
  osp_prompt="$(check_osp_prompt)"
  ps_venv_prompt="$(set_venv_color)"
  # Set prompt
  PS1="$ps_venv_prompt$ps_rebbot$ps_username$ps_uh_separator$ps_hostname$osp_prompt $ps_path $ps_scm_prompt$ps_user_mark$ps_user_input"
}
